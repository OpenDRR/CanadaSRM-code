    ...
    for out in outputs:
        for li, ln in enumerate(xtypes):
            ...
            export_df = pandas.DataFrame({
                ...
                'variance': alt.variance.to_numpy()
            })
...
def ebrisk(proxies, cmaker, stations, dstore, monitor):
    ##################### BELOW ADDED TH AUG 2026 #########################
    print(
            f"******** EBRISK CORE TASK CALLED: "
            f"PID={os.getpid()} ********",
            flush=True
    )
    with open("/scratch/parquet-out/ebrisk_called.txt", "a") as f:
        f.write(
            f"ebrisk called: PID={os.getpid()}, "
            f"cwd={os.getcwd()}, "
            f"file={__file__}\n"
        )
    ##################### ABOVE ADDED TH AUG 2026 #########################
    """

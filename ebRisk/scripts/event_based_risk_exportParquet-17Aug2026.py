...
####################### BELOW ADDED BY TH JUNE 2026 #######################################
import os
import uuid
from datetime import date

# Target approximately 300 MB of uncompressed pandas data per output file
EXPORT_TARGET_BYTES = 300 * 1024**2

# Each worker process gets its own buffer
_EXPORT_BUFFER = []
_EXPORT_BUFFER_BYTES = 0
_EXPORT_FILE_NUMBER = 0
####################### ABOVE ADDED BY TH JUNE 2026 #######################################
...
####################### BELOW ADDED BY TH JUNE 2026 #######################################
def write_export(df):
    global _EXPORT_BUFFER
    global _EXPORT_BUFFER_BYTES
    global _EXPORT_FILE_NUMBER

    if df.empty:
        return

    _EXPORT_BUFFER.append(df)
    _EXPORT_BUFFER_BYTES += df.memory_usage(deep=True).sum()

    # Only write once the worker's buffer reaches ~300 MB
    if _EXPORT_BUFFER_BYTES >= EXPORT_TARGET_BYTES:
        export_df = pandas.concat(_EXPORT_BUFFER, ignore_index=True)

        fname = (
            f"/scratch/parquet-out/asset_event_losses_"
            f"{date.today()}_"
            f"{os.getpid()}_"
            f"{_EXPORT_FILE_NUMBER:06d}_"
...
def aggreg(outputs, crmodel, ARK, aggids, rlz_id, ideduc, monitor):
    ...
    for out in outputs:
        for li, ln in enumerate(xtypes):
            ...
            alt = out[ln]
            ############################# BELOW ADDED BY TH IN JUNE 2026 ###############################
            export_df = pandas.DataFrame({
                'event_id': alt.eid.to_numpy(),
                'aid': alt.aid.to_numpy(),
                'loss_type': ln,
                'loss': alt.loss.to_numpy()
            })
            export_df = export_df[export_df['loss'] >= 5000]
            write_export(export_df)
            ############################# ABOVE ADDED BY TH IN JUNE 2026 ###############################
            if oq.avg_losses:
                with mon_avg:
                    coo = average_losses(
...
def ebrisk(proxies, cmaker, stations, dstore, monitor):
    ...
    ##################### BELOW ADDED BY TH AUG 2026 ######################
    flush_export()
    ##################### ABOVE ADDED BY TH AUG 2026 ######################
...
class EventBasedRiskCalculator(event_based.EventBasedCalculator):
    ...
    def save_tmp(self, monitor):
        ...
        ####################### BELOW MODIFIED BY TH JUNE 2026 #######################################
        #del adf['id']
        ####################### ABOVE MODIFIED BY TH JUNE 2026 #######################################
